    ...
    PetscInt Xf, PetscInt Yf, PetscInt Xc, PetscInt Yc)
{
    ...
    si += 1.0 / __ldg(&Rf[if0*Yf + jf0]); cnt++;
    if (jf1 != jf0) { si += 1.0 / __ldg(&Rf[if0*Yf + jf1]); cnt++; }
    if (if1 != if0) { si += 1.0 / __ldg(&Rf[if1*Yf + jf0]); cnt++; }
    if (if1 != if0 && jf1 != jf0) { si += 1.0 / __ldg(&Rf[if1*Yf + jf1]); cnt++; }
...
void launch_matvec(
    ...
    PetscInt above_halo_start_local, PetscInt below_halo_start_global, PetscInt below_halo_start_local)
{
    ...
    PetscInt r_start, PetscInt r_end, PetscInt Xl, PetscInt Yl)
{
    ...
    int threads = 256;
    int blocks  = ((int)lrows + threads - 1) / threads;
    diagonal_kernel<<<blocks, threads>>>(d_dev, d_R, r_start, r_end, Xl, Yl);
}
    ...
    PetscInt N, bool is_heating)
{
    ...
    update_rgrid_kernel<<<blocks, threads>>>(d_R, d_Tthresh, temp, ins_R, N, is_heating);
}

void launch_gather(double* d_dst, const double* d_src, const int* d_indices, int count)
{
    int threads = 256;
    int blocks  = (count + threads - 1) / threads;
    if (blocks > 0) gather_kernel<<<blocks, threads>>>(d_dst, d_src, d_indices, count);
}

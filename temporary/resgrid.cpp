...
static PetscErrorCode levelMatvec(Mat M, Vec x, Vec y)
{
    ...
    // Because both are VECCUDA, this copies directly on the device!
    VecCopy(x, ctx->x_ghost);

    // --- Step 2: MPI ghost exchange ---
    // PETSc automatically uses CUDA-aware MPI or handles staging internally
                                        ...
                                        MPI_Comm comm)
{
...

    {
        ...
        IS from_is, to_is;
        VecSetType(fine_mpi, VECCUDA); // ADD THIS LINE
...

    {
        ...
        VecCreateMPI(comm, coarse->r_end - coarse->r_start,
                     coarse->n, &coarse_mpi);
        VecSetType(coarse_mpi, VECCUDA); // ADD THIS LINE
...
static PetscErrorCode ShellCreateVecs(Mat mat, Vec *right, Vec *left) {
    PetscInt m, n, M, N;
    ...
    // The 'right' vector size matches matrix columns (n, N)
    if (right) {
        VecCreateMPI(comm, n, N, right);
        VecSetType(*right, VECCUDA);
    }
    // The 'left' vector size matches matrix rows (m, M)
    if (left) {
        VecCreateMPI(comm, m, M, left);
        VecSetType(*left, VECCUDA);
    }
    return 0;
}
...
static void buildLevelMat(MGLevel& lv, MPI_Comm comm)
{
    ...
    MatShellSetOperation(lv.mat, MATOP_CREATE_VECS, (void(*)(void))ShellCreateVecs);
                                  ...
                                  PCMGSetupData& setup, MPI_Comm comm)
{
    ...
    for (int lv = 0; lv < nlevels-1; lv++) {
        ...
        MatShellSetOperation(setup.R_mats[lv], MATOP_CREATE_VECS, (void(*)(void))ShellCreateVecs);
